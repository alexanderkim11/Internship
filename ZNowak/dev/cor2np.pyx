...
from libc.stdio cimport FILE, fopen, fseek, fclose, SEEK_END, SEEK_SET, ftell, fread, fgets
...
cdef void _insert_in_hashmap(char* utf8_string, int length, int file_id, int num_files):
    ...
    if temp is not NULL:
        ...
        if value is not NULL: # Update
            value[file_id] = value[file_id] + 1
            return
        value = <int*> calloc(num_files, sizeof(int))
        #free(value)
        #numbers = <int>8
        #value = <int*> numbers
        #c = array.clone(a, num_files, zero=True)
        #value = <int*>c.data.as_voidptr
		#if value is NULL:
		#	print("NULL\n")
		#	return
        value[file_id] = 1
        hashmap.set(key, value)
        #print(len(hashmap), value[0], file_id, num_files)
...
def corpus_to_array(list filenames, list nouns):
    ...
    for i in range(num_files):
        ...
        for word in words:
            #_insert_in_nounmap(noun.lower(), len(noun))
            _insert_in_hashmap(word.lower(), len(word), i, num_files) 
